...
def build_migration_seed():
    ...
    if sql_dir.exists():
        print(f"[INFO] Cleaning output directory: {sql_dir}")
    ...
    for seed_file in seed_dir.glob("*seed.json"):
        # if seed_file.name not in ["00__seed.json"]:
        # continue
        ...
        if "dependencies" in data:
            ...
            for dep in data["dependencies"]:
                for slave_id in dep["slave_ids"]:
                    sql.append(
                        DEPENDENCY_SQL.format(master=dep["master_id"], slave=slave_id)
                    )
...
def process_geometry(geometries: list) -> list[str]:
    ...
    VALUES (new_id, '{geom['geotype']}', {geom.get('angle', 0)}, {geom_sql});"""
            ...
            coords = ", ".join(
                [f"ST_MakePoint({x}, {y})" for x, y in geom["figure"]["coordinates"]]
            )
